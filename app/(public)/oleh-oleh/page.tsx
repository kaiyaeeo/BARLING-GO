    ...
    return (
        ...
        <main className="min-h-screen bg-white pt-16">
            ...
            <div className="mb-3">
                ...
                {KABUPATEN.map((kab) => (
                    ...
                    className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-all ${
                        kabupaten === kab ? "bg-[#2D7D46] text-white border-[#2D7D46]" : "bg-white text-gray-600 border-gray-200 hover:border-gray-400"
                    }`}>{kab}</Link>
                ))}
                </div>
            </div>
            ...
            {items.length === 0 ? (
                ...
                <Link href="/oleh-oleh" className="mt-3 inline-block text-sm text-[#2D7D46] hover:underline">Reset filter</Link>
                </div>
            ) : (
            ...
            {totalPages > 1 && (
                ...
                {[1,2,3].filter(p => p <= totalPages).map((p) => (
                    ...
                    className={`w-10 h-10 flex items-center justify-center rounded-full text-sm font-semibold transition-all ${p === page ? "bg-[#2D7D46] text-white" : "border border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
                    {p}
                    </Link>
                ))}
