        ...
        <main className="min-h-screen bg-white pt-16">
            ...
            <form method="GET" className="mb-6">
                ...
                <input
                    ...
                    className="w-full pl-12 pr-5 py-3 rounded-2xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#2D7D46]/30 focus:border-[#2D7D46] bg-gray-50"
                />
            ...
            {!items || items.length === 0 ? (
                ...
                <Link href="/kuliner" className="mt-3 inline-block text-sm text-[#2D7D46] hover:underline">Reset filter</Link>
                </div>
            ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
                {items.map((item: any) => (
                    <Link key={item.id} href={`/kuliner/${item.slug}`} className="group block bg-white border border-gray-100 rounded-2xl overflow-hidden hover:shadow-lg transition-all">
                    <div className="relative aspect-[4/3] overflow-hidden">
                        <img src={imgSrc(item)} alt={item.title}
                        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                        {(item.review_count ?? 0) >= 8 && (
                        <span className="absolute top-2.5 left-2.5 text-xs font-bold text-white bg-[#FF6B35] px-2.5 py-1 rounded-full">Terlaris</span>
                        )}
                    ...
                    <div className="p-4">
                        <p className="text-xs font-bold text-[#2D7D46] uppercase tracking-wide mb-1">KULINER</p>
                        <h3 className="text-sm font-bold text-gray-900 mb-1 group-hover:text-[#2D7D46] transition-colors line-clamp-1">
                        {item.title}
                        </h3>
                        {item.kabupaten && (
                        ...
                        <p className="text-sm font-bold text-[#2D7D46]">
            ...
            {totalPages > 1 && (
                ...
                {[1, 2, 3].filter(p => p <= totalPages).map((p) => (
                    ...
                    className={`w-10 h-10 flex items-center justify-center rounded-full text-sm font-semibold transition-all ${
                        p === page ? "bg-[#2D7D46] text-white" : "border border-gray-200 text-gray-600 hover:bg-gray-50"
                    }`}>{p}</Link>
                ))}
