    ...
    return (
        ...
        {/* Left branding panel */}
        <div className={`hidden lg:flex lg:w-5/12 flex-col justify-between p-12 transition-colors duration-300 ${isSeller ? "bg-[#1a3a2a]" : "bg-[#2D7D46]"}`}>
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-12 bg-white">
            ...
            <p className="text-sm text-gray-500 mb-8">
                {isSeller ? <>Belum punya akun seller? <Link href="/register?mode=seller" className="text-[#2D7D46] font-semibold hover:underline">Daftar sebagai Seller</Link></> : <>Belum punya akun? <Link href="/register" className="text-[#2D7D46] font-semibold hover:underline">Daftar sekarang</Link></>}
            </p>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="nama@email.com" required className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#2D7D46]/30 focus:border-[#2D7D46] transition-all" />
                </div>

                <div>
                <div className="flex justify-between mb-1.5">
                    <label className="text-sm font-medium text-gray-700">Password</label>
                    <Link href="/forgot-password" className="text-xs text-[#2D7D46] hover:underline">Lupa password?</Link>
                </div>
                <div className="relative">
                    <input type={showPass ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Minimal 6 karakter" required className="w-full px-4 py-3 pr-11 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#2D7D46]/30 focus:border-[#2D7D46] transition-all" />
                ...
                <button type="submit" disabled={loading} className={`w-full py-3 font-bold rounded-xl text-sm text-white flex items-center justify-center gap-2 transition-all disabled:opacity-60 ${isSeller ? "bg-[#1a3a2a] hover:bg-[#0f2619]" : "bg-[#2D7D46] hover:bg-[#236338]"}`}>
