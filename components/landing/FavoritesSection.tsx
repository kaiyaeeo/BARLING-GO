    ...
    return (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {products.length === 0 ? (
                ...
                return (
                    <Link key={item.id} href={`/produk/${item.slug}`} className="group relative rounded-2xl overflow-hidden aspect-square cursor-pointer">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-3 translate-y-full group-hover:translate-y-0 transition-transform">
                        ...
                        {item.categories && <span className="text-xs text-white/70 capitalize">{item.categories.name}</span>}
                    </div>
                    </Link>
                )
                })
            ...
            <Link href="/produk" className="px-8 py-3 text-sm font-semibold text-gray-800 border border-gray-300 rounded-xl hover:bg-gray-50 hover:border-gray-400 transition-all">
                Explore More
            </Link>
            </div>
        </div>
