    ...
    if (status === "paid") {
        ...
        <button onClick={() => updateStatus("processing")}
            className="px-3 py-1.5 bg-[#2D7D46] hover:bg-[#236338] text-white text-xs font-bold rounded-lg transition-all">
            Proses
        </button>
        )
    }
    ...
    if (status === "delivered") {
        ...
        <Link href={`/admin/pesanan/${orderId}`}
            className="px-3 py-1.5 text-[#2D7D46] text-xs font-bold rounded-lg hover:bg-green-50 transition-all">
            Lihat Detail
        </Link>
        )
    }
