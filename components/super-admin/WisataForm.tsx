    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        setSaved(true)
        setTimeout(() => router.push("/super-admin/kelola-wisata"), 1000)
    }
    ...
    return (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 space-y-4">
            ...
            <div>
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#2D7D46]/30 bg-white">
            ...
            <div className="flex items-center justify-between mb-1.5">
                ...
                className="flex items-center gap-1.5 text-xs font-medium text-[#2D7D46] hover:underline disabled:opacity-50">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <button type="button" onClick={() => fileRef.current?.click()} disabled={uploading}
                className="w-full h-32 border-2 border-dashed border-gray-200 rounded-xl flex flex-col items-center justify-center gap-2 hover:border-[#2D7D46] hover:bg-green-50 transition-all text-gray-400 hover:text-[#2D7D46]">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
            ...
            <input type="checkbox" name="is_published" checked={form.is_published}
                onChange={handleChange} className="w-4 h-4 accent-[#2D7D46]" />
            <div>
                <p className="text-sm font-medium text-gray-800">Publikasikan Sekarang</p>
                <p className="text-xs text-gray-400">Destinasi akan langsung tampil di halaman publik</p>
            </div>
            </label>
        </div>
        ...
        <div className="flex items-center gap-3 pt-2">
            ...
            {saved   && <Check size={14} />}
            {saved ? "Tersimpan!" : saving ? "Menyimpan..." : isEdit ? "Simpan Perubahan" : "Tambah Destinasi"}
            </button>
            <button type="button" onClick={() => router.back()}
            className="px-6 py-3 border border-gray-200 text-gray-600 font-medium rounded-xl text-sm hover:bg-gray-50 transition-all">
            Batal
            </button>
        </div>
        </form>
