    ...
    return (
        ...
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center mb-5 border-b border-gray-100 pb-4">
                ...
                className={`w-11 h-6 rounded-full relative transition-colors ${formBackup.autoBackup ? "bg-[#2D7D46]" : "bg-gray-300"}`}
                >
            ...
            <div className="space-y-4">
                ...
                <select 
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none bg-white focus:border-[#2D7D46]"
                >
                ...
                <div>
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-[#2D7D46]" 
                    />
                </div>
                <div>
                    ...
                    className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-sm outline-none bg-white focus:border-[#2D7D46]"
                    >
            ...
            <div className="space-y-3 mb-6">
                {/* Opsi SQL */}
                <div onClick={() => toggleManualBackup('sql')} className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${formBackup.manualOptions.sql ? 'border-[#2D7D46] bg-green-50/30' : 'border-gray-200 bg-white'}`}>
                <div className={`mt-0.5 w-4 h-4 rounded flex items-center justify-center border shrink-0 ${formBackup.manualOptions.sql ? 'bg-[#2D7D46] border-[#2D7D46] text-white' : 'bg-white border-gray-300'}`}>
                ...
                {/* Opsi Media */}
                <div onClick={() => toggleManualBackup('media')} className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${formBackup.manualOptions.media ? 'border-[#2D7D46] bg-green-50/30' : 'border-gray-200 bg-white'}`}>
                <div className={`mt-0.5 w-4 h-4 rounded flex items-center justify-center border shrink-0 ${formBackup.manualOptions.media ? 'bg-[#2D7D46] border-[#2D7D46] text-white' : 'bg-white border-gray-300'}`}>
                ...
                {/* Opsi Log */}
                <div onClick={() => toggleManualBackup('log')} className={`flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${formBackup.manualOptions.log ? 'border-[#2D7D46] bg-green-50/30' : 'border-gray-200 bg-white'}`}>
                <div className={`mt-0.5 w-4 h-4 rounded flex items-center justify-center border shrink-0 ${formBackup.manualOptions.log ? 'bg-[#2D7D46] border-[#2D7D46] text-white' : 'bg-white border-gray-300'}`}>
            ...
            <button type="button" onClick={() => alert("Memulai proses pencadangan (backup) instan...")} className="w-full py-3 bg-[#1A4C2E] hover:bg-[#11331e] text-white font-bold text-sm rounded-xl flex items-center justify-center gap-2 transition-colors">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col h-full">
            ...
            <button type="button" className="text-sm font-semibold text-[#2D7D46] flex items-center gap-1.5 hover:underline">
            ...
            <table className="w-full text-sm text-left">
                ...
                {[
                    ...
                    <td className="py-4 text-right">
                        {row.status === "Sukses" && (
                        <button type="button" className="text-[#2D7D46] hover:bg-green-50 p-1.5 rounded-lg transition-colors" title="Unduh File Backup">
                            <Download size={16} />
                        </button>
                        )}
                    </td>
        ...
        <div className="lg:col-span-2 flex justify-end gap-3 pt-4 border-t border-gray-100 mt-2">
            ...
            <button type="submit" disabled={isSaving} className="flex items-center gap-2 px-6 py-2.5 bg-[#2D7D46] text-white font-semibold text-sm rounded-xl hover:bg-[#1A4C2E] transition-all">
